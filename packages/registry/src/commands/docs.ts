...
export const docsCommand = new Command('docs')
  ...
  .action(async (componentDir: string, options: any) => {
    try {
      ...
      const docs = generateDocumentation(metadata, options.platform);
      
      // Write documentation to file
      const outputPath = path.join(absoluteComponentDir, options.output);
 ...
 */
function generateDocumentation(metadata: any, platform: string): string {
  const { name, description, dependencies, files } = metadata;
  
  // Find component file
  const componentFile = files.find((file: any) => 
    file.name.endsWith('.tsx') || file.name.endsWith('.jsx')
  );
  
  // Find types file
  const typesFile = files.find((file: any) => file.name === 'types.ts');
